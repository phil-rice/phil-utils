...
trait CepState {
  ...
  def processAtEnd[ED](trieMap: TrieMap[Any, StoredState])(pipelineData: PipelineData): Option[StoredState]
}

object Terminate extends CepState {
  override def list: List[StatePipeline] = List()
  override def name: String = "terminate"
  override def processAtEnd[ED](trieMap: TrieMap[Any, StoredState])(pipelineData: PipelineData): Option[StoredState] = {trieMap.remove(pipelineData.key); None}
}
...
case class UserState(name: String, var list: List[StatePipeline]) extends CepState {
  ...
  override def processAtEnd[ED](trieMap: TrieMap[Any, StoredState])(pipelineData: PipelineData): Option[StoredState] =
